...
// MARK: - Simple Tempo Dial Component
struct TempoDialView: View {
    ...
    var body: some View {
        ZStack {
            // Single dial circle
            Circle()
                .fill(Color(white: 0.005))
                .frame(width: size * 0.7, height: size * 0.7)
            
            // Rotating indicator dot (this rotates)
            Circle()
                .fill(Color.white.opacity(0.8))
                .frame(width: size * 0.025, height: size * 0.025)
                .offset(y: -(size * 0.7 / 2 - size * 0.06))
        ...
        .gesture(
            DragGesture()
                .onChanged { value in
                    let center = CGPoint(x: size * 0.8/2, y: size * 0.8/2)
