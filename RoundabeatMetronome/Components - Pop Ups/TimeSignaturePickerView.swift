...
struct TimeSignaturePickerView: View {
    ...
    var body: some View {
        ...
        .frame(width: 360, height: 600) // Fixed size instead of maxWidth/maxHeight
        .shadow(color: Color.black.opacity(0.3), radius: 20, x: 0, y: 10)
        .onAppear {
            // Initialize custom values with current time signature
            customNumerator = metronome.beatsPerMeasure
            customDenominator = metronome.beatUnit
        }
    }
    ...
    private var customTimeSignatureSection: some View {
        VStack(spacing: 15) { // Reduced from 20 to 15
            ...
            HStack(spacing: 25) { // Reduced from 30 to 25
                // Numerator picker
                VStack(spacing: 6) { // Reduced from 8 to 6
                    ...
                    Picker("Numerator", selection: $customNumerator) {
                        ForEach(1...32, id: \.self) { num in
                            Text("\(num)")
                                .font(.custom("Kanit-Medium", size: 16)) // Reduced from 18 to 16
                                .foregroundColor(Color.white.opacity(0.8))
                                .tag(num)
                        }
                    }
                    .pickerStyle(WheelPickerStyle())
                    .frame(width: 70, height: 100) // Reduced height from 120 to 100
                    .clipped()
                    .background(
                ...
                Text("/")
                    .font(.custom("Kanit-Regular", size: 28)) // Reduced from 32 to 28
                    .foregroundColor(Color.white.opacity(0.6))
            ...
            Button(action: {
                ...
                Text("APPLY CUSTOM")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 20) // Reduced from 25 to 20
                            .fill(Color.white.opacity(0.15))
                            .overlay(
                                RoundedRectangle(cornerRadius: 20) // Reduced from 25 to 20
                                    .stroke(LinearGradient(
